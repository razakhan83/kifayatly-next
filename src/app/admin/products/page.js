    ...
    try {
      ...
      if (data.success) {
        setProducts((previous) =>
          previous.map((entry) => (entry._id === product._id ? { ...entry, isLive: !entry.isLive } : entry)),
        );
        toast.success(`"${product.Name}" is now ${!product.isLive ? "Live" : "Draft"}.`);
      } else {
  ...
  return (
    <div className="pb-24 md:pb-0">
      ...
      <div className="space-y-4 md:hidden">
        {loading ? (
          [...Array(3)].map((_, index) => <div key={index} className="surface-card h-32 rounded-xl animate-pulse" />)
