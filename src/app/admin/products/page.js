  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showToast(
          `Product "${deleteModal.product.Name}" has been deleted.`,
          "error",
        );
      } else {
  ...
  const handleToggleLive = async (product) => {
    ...
    try {
      ...
      if (data.success) {
        setProducts((prev) =>
          prev.map((p) =>
            p._id === product._id ? { ...p, isLive: !p.isLive } : p,
          ),
        );
        showToast(
          `"${product.Name}" is now ${!product.isLive ? "Live" : "Draft"}.`,
          "success",
        );
      } else {
            ...
            >
              <div className="p-4">
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {(Array.isArray(product.Category)
                        ...
                        .map((c, i) => (
                          <span
                            key={i}
                            className="px-2 py-0.5 bg-gray-100 text-gray-600 text-[10px] font-bold rounded-lg uppercase tracking-wider"
                          >
                            {c}
                          </span>
                        ))}
                ...
                <div className="flex items-center justify-between mt-3 px-1">
                  <span
                    className={`text-xs font-bold ${product.isLive ? "text-emerald-600" : "text-gray-400"}`}
                  >
                  ...
                  >
                    <span
                      className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform duration-300 ${product.isLive ? "translate-x-5" : "translate-x-0"}`}
                    />
                  </button>
                </div>
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-sm font-semibold text-gray-900 line-clamp-2 max-w-[180px] leading-tight">
                            {product.Name}
                          </span>
                        </div>
                      </td>
                        ...
                        <div className="flex flex-wrap gap-1 max-w-[160px]">
                          {(Array.isArray(product.Category)
                            ...
                            .map((c, i) => (
                              <span
                                key={i}
                                className="px-2 py-0.5 bg-gray-100 text-gray-600 text-[10px] font-bold rounded-md uppercase"
                              >
                                {c}
                              </span>
                            ))}
                      ...
                      <td className="px-6 py-4">
                        {product.StockStatus === "In Stock" ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 text-emerald-700 text-xs font-bold rounded-full">
                            <i className="fa-solid fa-circle text-[4px]"></i> In
                            Stock
                          </span>
                        ) : (
      ...
      {deleteModal.open && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center">
          <div
            ...
            onClick={() =>
              !deleting && setDeleteModal({ open: false, product: null })
            }
          ></div>
