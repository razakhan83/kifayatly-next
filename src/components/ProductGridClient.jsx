...
function ProductGridContent({ initialProducts, forceSearchTerm, hideSearch }) {
  ...
  const [searchTerm, setSearchTerm] = useState(
    forceSearchTerm || searchParams?.get("search") || "",
  );
  ...
  const searchFilteredProducts = useMemo(() => {
    ...
    return base.filter((p) => {
      ...
      return (
        name.includes(term) ||
        categories.some((c) => c.toLowerCase().includes(term))
      );
    });
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (sortBy === "price-low") {
      ...
      base.sort(
        (a, b) => (b.Price || b.price || 0) - (a.Price || a.price || 0),
      );
    } else if (sortBy === "az") {
      base.sort((a, b) =>
      ...
      base.sort((a, b) =>
        (b.Name || b.name || "").localeCompare(a.Name || a.name || ""),
      );
  ...
  useEffect(() => {
    if (categoryNavRef.current) {
      const activePill =
        categoryNavRef.current.querySelector(".cat-pill-active");
      if (activePill) {
  ...
  return (
    <>
      ...
      <section
        id="products-section"
        className="section py-3 flex-grow w-full overflow-hidden"
      >
        <div className="container mx-auto px-2 max-w-7xl">
          ...
          <div className={`transition-opacity duration-300 ${fadeState}`}>
            <div className="products-grid grid auto-rows-max grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5">
          ...
          {hasMore && (
            <div
              ref={loadMoreRef}
              className="h-10 w-full border-t border-transparent"
            />
          )}
        </div>
      </section>
